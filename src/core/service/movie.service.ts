...
export class MovieService {
  ...
  async getMyMovies(): Promise<Movie[]> {
    try {
      const response = await this.movieRepository.getMyMovies();
      if (response.error) {
        throw new Error(response.error);
      }
      return response.data;
  ...
  async getMoviesWithBts(): Promise<Movie[]> {
    try {
      const response = await this.movieRepository.getMovieWithBts();
      if (response.error) {
        throw new Error(response.error);
      }
      return response.data;
    } catch (error) {
      console.error("Error in getMoviesWithBts:", error);
      throw error;
    }
  }
